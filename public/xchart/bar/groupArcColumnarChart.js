export default function buildOption(chartOption, registerTheme) {
  ...
  if (chartOption.series.length) {
    ...
    for (let i = 0; i < chartYData.length; i++) {
      ...
      buildSeriesData.push({
        ...
        barWidth: registerTheme?.arcColumnarBar?.barWidth,
        coordinateSystem: "polar",
        // stack: 'a',
        emphasis: {
          itemStyle: {
  ...
  option.barCategoryGap = registerTheme?.arcColumnarBar?.barCategoryGapAll
    ? registerTheme.arcColumnarBar.barCategoryGapAll
    : "30%";
  //     let bar1 = JSON.parse(JSON.stringify(registerTheme.arcColumnarBar));
  //      console.log(bar1,"bar1");
  //    bar1.barRadius = registerTheme?.arcColumnarBar?.itemStyle?.normal?.barBorderRadius ? registerTheme.arcColumnarBar.itemStyle.normal.barBorderRadius : [0, 0, 0, 0];
