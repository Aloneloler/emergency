...
//组合数据
  function sortNumber(a,b){return a - b} //从小到大排列
  function delRepeat( arr ){//去掉重复得
    // 定义一个新数组
    var newArr = [];
    // 遍历传进来的数组
  ...
  let barNumber =registerTheme?.bar?.barNumber
  let dataArray=[];//原始数据
 
  for (let m = 0; m < chartOption.series.length; m++) {
  for (let k = 0; k < chartOption.series[m].data.length; k++) {
    dataArray.push(Number(chartOption.series[m].data[k] ));
  }
}
  dataArray.sort(sortNumber);
  dataArray=delRepeat(dataArray)
  let max = Math.max.apply(null, dataArray).toFixed(0);
  let min = Math.min.apply(null, dataArray).toFixed(0);
  let divide=((max-min+1)/barNumber).toFixed(0);//等分
  let arrNew = [];//区间范围
  let xarrNew = [];
  for(let j=0;j<barNumber;j++)
  {
    let x1 = Number(j * divide)+Number(min);
    let x2 = Number((j + 1) * divide)+Number(min);
  ...
  for(let r=0;r<arrNew.length;r++)
  {
    // let array=[];
    // for(let i=0;i<dataArray.length;i++)
    // {
    //      if(arrNew[r][0]<=dataArray[i]&&dataArray[i]<arrNew[r][1])
    //      {
    //       array.push(dataArray[i]);
    //      }
    // }
    //valueArray.push(array.length);
    if((arrNew.length-r)===1)
    {
      xarrNew.push("["+arrNew[r].join()+"]")
    }else{
      xarrNew.push("["+arrNew[r].join()+")")
    }
  }
  //构建X轴
  option.xAxis[0].data = xarrNew;
  ...
  for (let i = 0; i < chartOption.series.length; i++) {
    ...
    let buildData = [];
    let valueArray=[];//区间得个数
    for(let r=0;r<arrNew.length;r++)
    {
      let array=[];
      for(let k=0;k<chartOption.series[i].data.length;k++)
      {
           if(arrNew[r][0]<=Number(chartOption.series[i].data[k])&&Number(chartOption.series[i].data[k])<arrNew[r][1])
           {
            array.push(Number(chartOption.series[i].data[k]));
           }
      }
      valueArray.push(array.length);
    }
    for (let k = 0; k < valueArray.length; k++) {
      buildData.push({ name: "", value: valueArray[k] });
    }
    //console.log(chartOption.series[i].data)
    buildSeries.push({
      barGap: 0,
      barCategoryGap:0,
