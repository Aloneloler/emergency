...
for (let k = 0; k < chartOption.series[j].data.length; k++) {
  arrayData.push(Number(chartOption.series[j].data[k]));
}
...
//保存结果的数组
let result = [];  
for(let key in totalData ){//每个数组第一个之和
  //遍历数组的每一项
  totalData[key].forEach((value,index) => {
  ...
  for (let i = 0; i < chartOption.series.length; i++) {
    ...
    for (let k = 0; k < chartOption.series[i].data.length; k++) {
        ...
        buildData.push({ name: "", value: (chartOption.series[i].data[k]*(100/result[k])).toFixed(2)});
      }
    buildSeries.push({
