...
export default function buildOption(chartOption, registerTheme) {
  ...
  //console.log(chartOption, 'option2222222222')
  //构建series
  let buildSeries = [];
  let buildLegends = [];
  ...
  for (let i = 0; i < chartOption.series.length; i++) {
    ...
    }else if(chartOption.yAxisLengthArray[0]>0&&i <chartOption.yAxisLengthArray[0]){
      for(let k = 0; k < chartOption.yAxisLengthArray[0]; k++)
      {
        dataArray = [];
        let seriesObj = chartOption.series[k];
        for (let j = 0; j < seriesObj.data.length; j++) {
          dataArray.push(-seriesObj.data[j]);
        }
        //console.log(dataArray,k,chartOption.yAxisLengthArray[0], 'dataArray')
      }
    }
